package se.ade.mc.cubematic.placing

import se.ade.mc.cubematic.*
...
class PlacingAspect(private val plugin: CubematicAutomationPlugin) : Listener {
    ...
    fun onEvent(event: PlayerInteractEvent) {
        ...
        if(dispenser.customName == null) {
            dispenser.customName = PLACER_NAME
            dispenser.update()
        }
    ...
    fun onEvent(event: BlockBreakEvent) {
        ...
        val dispenser = block.toDispenser()
        if(dispenser.isPlacer(plugin)) {
            if(dispenser.customName == PLACER_NAME) {
                dispenser.customName = null
            }

            dispenser.setIsPlacer(false, plugin)

            block.world.dropItem(block.location.getCenter(), ItemStack(Material.SHULKER_SHELL, 1))
